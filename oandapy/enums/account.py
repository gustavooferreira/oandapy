...
class AccountFinancingMode(Enum):
    ...
    NO_FINANCING        No financing is paid/charged for open Trades in the
                        Account.
    SECOND_BY_SECOND    Second-by-second financing is paid/charged for open
                        Trades in the Account, both daily and when the the
                        Trade is closed.
...
class PositionAggregationMode(Enum):
    ...
    ABSOLUTE_SUM    The Position value or margin for each side (long and short)
                    of the Position are computed independently and added
                    together.
    MAXIMAL_SIDE    The Position value or margin for each side (long and short)
                    ...
                    value or margin chosen is the maximal absolute value of the
                    two.
    NET_SUM         The units for each side (long and short) of the Position
                    are netted together and the resulting value (long or short)
                    is used to compute the Position value or margin.
    """
